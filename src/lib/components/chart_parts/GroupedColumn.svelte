<!--
  @component
  Generates an SVG column chart.
 -->
<script>
  //This component is based heavily on this great (but uncompiling) example from git user techniq:
  //repl: https://svelte.dev/playground/d5d3eef9aa9a42e4b07c0a3b917a3242?version=3.17.3

  import { getContext } from "svelte";

  const { data, xGet, yGet, zGet, zScale, x, yRange, xScale, y, height, custom } =
    getContext("LayerCake");

  /** @type {String} [stroke='#000'] - The shape's stroke color. */
  export let stroke = "#000";

  /** @type {Number} [strokeWidth=0] - The shape's stroke width. */
  export let strokeWidth = 0;

  /** @type {boolean} [false] - Show the numbers for each column */
  export let showLabels = false;

  $: columnWidth = (d) => {
    const vals = $xGet(d);
    return Math.abs(vals[1] - vals[0]);
  };

  $: $zScale.range([0, $xScale.bandwidth()]);

  $: columnHeight = (d) => {
    return $yRange[0] - $yGet(d);
  };
</script>

<g class="column-group">
  {#each $data as series, i}
    <g style="transform: translate({$xGet(series)}px,0)">
      {#each series.values as d, i}
        {@const colHeight = columnHeight(d)}
        {@const colWidth = $zScale.bandwidth ? $zScale.bandwidth() : columnWidth(d)}
        {@const xGot = $zGet(d)}
        {@const xPos = Array.isArray(xGot) ? xGot[0] : xGot}
        {@const yValue = $y(d)}
        <rect
          class="group-rect"
          data-id={i}
          x={$zGet(d)}
          y={$yGet(d)}
          width={$zScale.bandwidth()}
          height={colHeight}
          fill={$custom.seriesColors[i]}
          {stroke}
          stroke-width={strokeWidth}
        />
        {#if showLabels && yValue}
          <text x={xPos + colWidth / 2} y={$height - colHeight - 5} text-anchor="middle"
            >{$custom.format.format(yValue)}</text
          >
        {/if}
      {/each}
    </g>
  {/each}
</g>

<style>
  text {
    font-size: 12px;
  }
  rect {
    transition: ease-in-out 250ms;
  }
</style>
